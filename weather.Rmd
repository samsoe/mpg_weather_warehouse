...
renderPlot({
  show_weather_df() %>%
    ...
    scale_color_discrete_sequential(name = "Station", palette = "Viridis") +
    theme_bw() +
...
renderPlot({
  show_weather_df() %>%
    ...
    geom_point(aes(fill = station), shape = 21, color = "black", stroke = 0.4, alpha = 0.6, size = 1.2) +
    facet_grid(vars(year), as.table = FALSE) +
    labs(x = "", y = "Prevailing Wind Direction") +
    scale_x_date(labels = function(x) format(x, "%b-%d")) +
    scale_y_continuous(labels = ordinal_directions, breaks = (0:4) * 90, limits = c(0, 360)) +
    scale_fill_discrete_sequential(name = "Station", palette = "Viridis") +
    theme_bw() +
    theme(legend.position="top")
})
```
