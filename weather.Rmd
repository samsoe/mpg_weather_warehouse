...
renderPlot({
  show_weather_df() %>%
    ...
    facet_grid(vars(year), as.table = FALSE) +
    labs(x = "", y = "Cumulative Rainfall (in)", caption = "The weather stations do not measure snow-water-equivalent. Values shown here depict rainfall, with trivial contributions from snow.") +
    scale_x_date(labels = function(x) format(x, "%b-%d")) +
    station_cols +
    theme_bw() +
    theme(legend.position="top", panel.grid.minor = element_blank())
